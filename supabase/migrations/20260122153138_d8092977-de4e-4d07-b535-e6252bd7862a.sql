...
-- Create a single RPC function that returns all shared dashboard data
-- This is the ONLY function needed for public dashboard access

CREATE OR REPLACE FUNCTION public.get_shared_dashboard_data(share_token text)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF share_token IS NULL OR length(share_token) != 32 OR share_token !~ '^[a-f0-9]+$' THEN
  ...
  WHERE sp.dashboard_share_token = share_token
